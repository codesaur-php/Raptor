...
- **Three Discord notifications were never sent: new public order, new signup request, user approved.** The dispatching controllers and `DiscordListener` disagreed on the event action string, and the listener silently ignores an action it does not branch on - so each event reached the listener and vanished without an error, log entry or failed request. `Web\Shop\ShopController::orderSubmit()` sent `OrderEvent('insert')` where `onOrderEvent()` handles `'new'` / `'status_changed'`; `LoginController::signup()` sent `UserEvent('signup')` and `UsersController::signupApprove()` sent `UserEvent('approve')` where `onUserEvent()` handles `'signup_request'` / `'approved'`. The e-mails those flows send were unaffected; only the webhooks were lost. The three controllers now dispatch the action the listener (and `docs/{en,mn}/api.md`, and the existing listener tests) already specified. `tests/Unit/Notification/EventDispatchServiceTest.php` gained a static scan over `OrderEvent`, `UserEvent` and `DevRequestEvent`: every `new XEvent('...')` under `application/` must use an action that the matching `DiscordListener` method branches on, so a dispatcher/listener mismatch fails CI instead of dropping notifications in production (`ContentEvent` is excluded - its unknown actions fall back to `contentAction()`).
- **Settings changes reached Discord as a generic content notification.** `DiscordNotifier::settingsUpdated()` (the section-aware embed) existed but `DiscordListener::onContentEvent()` never routed to it, so `SettingsController`'s `ContentEvent(module='settings')` fell through to `contentAction()` and read "*Admin* updated a settings". The listener now routes `module='settings'` to `settingsUpdated()` with the event title as the section (`texts` / `files` / `options`). `docs/{en,mn}/api.md` list the dedicated routes of `onContentEvent()` and no longer mention a `'review'` order action - product reviews travel as `ContentEvent` (`module='review'`).

### Changed

- **Every Markdown document was audited line by line against the source and brought back to the current state of the code** (`docs/{en,mn}/api.md`, `docs/{en,mn}/README.md`, `README.md`, `CLAUDE.md`, `database/migrations/README.md`, `.github/CONTRIBUTING.md`, `.github/SECURITY.md`). The reference docs had not followed the last releases and about 110 statements contradicted the code. The corrections that matter when reading the docs: `DiscordListener::__construct()` takes a `DiscordNotifier` (not a webhook URL), `DevRequestEvent` actions are `'new'` / `'updated'`, `ListenerProvider::listen()` (not `addListener()`), `EventDispatcher` accepts any `ListenerProviderInterface` and honours `isPropagationStopped()`, the `Event` base class owns `$user`; the web middleware pipeline includes `MethodOverride` and `BodyEncoding`, the public language comes from the URL prefix only (`/{xx}/*` mount, `/session/language/{code}` just redirects, no `/language/{code}` route), the public `/search` is `Web\Service\SearchController`; the full `WebRouter`, comment and HomeRouter/ManualRouter/ShopRouter route tables with their real names (`news-view`, `page-view`, `product-view`, `order-view`, `news-comment`, `news-comment-reply`, `dashboard-search`, `manual-view`, `product-review`); `Controller::template()` injects `csrf_token` / `waf_body_encoding` (no `request`) and registers the `pattern` filter; the `users`, `files`, `products`, `raptor_settings`, `raptor_menu`, `news_comments`, `messages` column tables (lengths, defaults, `review` instead of `comment` on products, no `is_active` on settings, `code='*'` language-neutral records); `ReferenceModel` / `RolePermission` / `Role` class names; `SpamProtectionTrait` signatures (`validateSpamProtection(array $parsed, string $formName, string $sessionKey, int $rateLimit = 10, int $minTime = 2): void`, `checkLinkSpam(string $text, int $maxLinks = 2): void`, `generateSpamToken()`); `SettingsController::updateEnv()` accepts only the four `RAPTOR_*_EMAIL_TO` names (the notification toggle is an empty address, no `*_NOTIFY` variables exist); `TextModel` is a `LocalizedModel` with `retrieve(?string $code = null)`; `AIHelper` requires a content/product insert-or-update permission, is rate-limited (30 calls / 60 s per user) and caps vision at 8 images; the Badge system has a fourth `info` color and does not exclude the admin's own actions for `/trash`; `CacheService::fromDefaultPath()` (used by `JWTAuthMiddleware` before the container exists), `has()` and the PSR-16 bulk methods; `PermissionsSeed` seeds 26 permissions (no `templates_index`), the role scopes of `RolePermissionSeed`, the 4-section `MenuSeed`; Trash: `deleteById()` runs before `store()`, restore pre-checks the original ID instead of catching `SQLSTATE 23000`, the whole restore runs in one transaction, `TrashController` is `system_coder`-only, the `users` / `organizations` log channels and the Users / Organizations "deactivate, then hard-delete into Trash" strategy; `MigrationSecurityScanner` also flags `CREATE TABLE`, `apply()` clears the cache, the per-user folder name is the sanitized username; the `.env` reference gained `RAPTOR_DB_DRIVER`, `RAPTOR_PASSWORD_RESET_MINUTES`, `RAPTOR_SIGNUP_VERIFY_HOURS`, `RAPTOR_MAIL_REPLY_TO_NAME`, the four `RAPTOR_*_EMAIL_TO` and the two `RAPTOR_OPENAI_*_MODEL` variables and lost `RAPTOR_JWT_LEEWAY` (nothing reads it - also removed from `docs/conf.example/.env.example`); the README examples now use mount-naive dashboard routes with `CsrfMiddleware`, `code IN (:code, '*')` with a bound parameter, `$model->insert()` returning the row, `'page'|link({'slug': ...})`, and the Template chapter documents autoescape and the `pattern` filter; `CLAUDE.md` names the per-user migration layout, the `{keyword}` fallback of `|text`, the sample-data modules, the `CREATE TABLE` scanner rule and the fact that `ci.yml` runs plain `composer validate --strict`. Code-side, the same pass replaced the raw `'mysql'` / `'pgsql'` strings in `DatabaseConnection` with `Constants::DRIVER_*` and refreshed three stale docblocks (`SessionMiddleware` closures, `AIHelper` route path, `UserEvent` actions).
