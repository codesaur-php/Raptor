...
- **CSRF protection** - Dashboard mutating routes (POST/PUT/PATCH/DELETE) are protected by the per-route `CsrfMiddleware` (login routes excepted); attach it to every new mutating route and use `csrfFetch()` for all state-changing requests in dashboard JS
