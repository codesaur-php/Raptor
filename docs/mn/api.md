...
`Dashboard\Application` нэмдэг: `HomeRouter`, `ShopRouter` (products + orders + reviews), `ManualRouter`, `Development\DevelopmentRouter` (хөгжүүлэлтийн хүсэлт `application/dashboard/development/`-д байрлана), `Protected\ProtectedRouter`, `Badge\BadgeRouter` (sidebar badge систем `application/dashboard/badge/`-д байрлана).

#### `overrideDashboardLayout(string $filename, string $customFile): static`

`DashboardTrait`-ийн дотооддоо рендэрлэдэг 3 layout template-ийг (`dashboard.html`, `alert-no-permission.html`, `modal-no-permission.html`) developer-ийн өөрийн файлаар солино - `application/raptor/`-т гар хүрэх шаардлагагүй. App-ийн constructor-т `parent::__construct()`-ийн дараа дуудна; custom файл байхгүй бол `InvalidArgumentException` шидэнэ (fail-fast). Map нь `handle()` дээр `dashboard_layouts` request attribute болж, `DashboardTrait::layout()`-оор резолюц хийгдэнэ. Өөрийн route-тэй хуудас (login г.м.) энэ map-д хамаарахгүй - тэдгээрийг router override-оор солино.

```php
// application/{myapp}/Application.php
parent::__construct($response);
// raptor-ийн өөрийн `template/`-ээс ялгаатай фолдер нэр ашиглаж төөрөгдлөөс сэргийл.
...
**Файл:** `application/raptor/DatabaseConnection.php`

Бүх PDO холболтыг нэг газраас удирдах helper класс. HTTP entry point
(`public_html/index.php`) болон тестүүд бүгд
`\Raptor\DatabaseConnection::connect()`-ийг дуудаж нэгэн ижил PDO авна.
...
Controller-ууд `Raptor\Controller::__construct()` дотор `$this->pdo` болгон
автоматаар авна.
...
Dashboard-ийн mutating хүсэлтүүдэд CSRF token шалгах **per-route** middleware. Router дээр mutating route бүрд `->middleware([CsrfMiddleware::class])`-аар наагдана (`use Raptor\CsrfMiddleware;`).
...
Layout template-ийн эцсийн замыг тогтооно: `Application::overrideDashboardLayout()`-оор бүртгэсэн custom файл байвал (`dashboard_layouts` request attribute-аар ирнэ) түүнийг, үгүй бол `raptor/template/` доторх default-ийг буцаана.
...
**Файл:** `application/raptor/migration/MigrationSecurityScanner.php`
...
**Файл:** `application/raptor/notification/EventDispatcher.php`
...
**Файл:** `application/raptor/notification/ListenerProvider.php`
...
**Файл:** `application/raptor/notification/ContentEvent.php`
...
**Файл:** `application/raptor/notification/UserEvent.php`
...
**Файл:** `application/raptor/notification/OrderEvent.php`
...
**Файл:** `application/raptor/notification/DevRequestEvent.php`
